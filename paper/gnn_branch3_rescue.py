"""Train a pyramidal-only 3-class branch rescue GNN.

This is an optional correction head for the current v12 pyramidal pipeline.
It is trained only on the existing pyramidal train split and leaves the
interneuron models untouched. Unlike the existing apical-vs-basal GNN, this
head predicts all neurite classes for every branch:

    0 = axon   (SWC 2)
    1 = basal  (SWC 3)
    2 = apical (SWC 4)

The input features include the branch morphology, subtree-owner probabilities,
and the label/confidence produced by the current Stage2+old-GNN path. That
lets the model learn corrections for the actual inference distribution,
including apical branches that were initially labeled axon.
"""
...
from paper.gnn_inference import GNNState, load_gnn, score_morphology
...
def _load_split(model_dir: Path, data_dir: Path) -> tuple[list[Path], list[Path]]:
    ...
    train_names = payload["train"].get("pyramidal", [])
    test_names = payload["test"].get("pyramidal", [])
    swc_dir = data_dir / "pyramidal" / "swc"
...
def _subtree_owner_model(bundle: dict) -> object | None:
    ...
    if by_ct:
        return by_ct.get("pyramidal")
    return bundle.get("pyramidal_subtree_owner_model")
...
def _current_branch_predictions(
    morph: MorphologyBranches,
    bundle: dict,
    subtree_owner_map: dict[int, dict[str, float | int]],
    old_gnn_state: GNNState | None,
) -> tuple[list[int], list[float]]:
    """Reproduce the current branch label state just before Stage3."""
    model = None
    models_by_ct = bundle.get("models_by_cell_type")
    if models_by_ct:
        model = models_by_ct.get("pyramidal")
    else:
    ...
    for br in morph.branches:
        ...
        if model is not None:
            ...
            if is_binary_b1:
                p_axon = float(probs[classes.index(1)]) if 1 in classes else 0.0
                p_dend = float(probs[classes.index(0)]) if 0 in classes else 1.0 - p_axon
                if p_axon > 0.5:
                    best_label = 2
                    best_conf = p_axon
                else:
                ...
                for cls, prob in zip(classes, probs):
                    cls_i = int(cls)
                    if cls_i in valid_labels:
                        valid_probs[cls_i] = float(prob)
                if valid_probs:
        ...
        confs.append(float(best_conf))

    if old_gnn_state is not None and morph.branches:
        apical_owner_root = _best_apical_owner(subtree_owner_map)
        dendrite_labels = {lbl for lbl in labels if lbl in (3, 4)}
        should_run = apical_owner_root is not None or {3, 4}.issubset(dendrite_labels)
        if should_run:
            gnn_preds = score_morphology(old_gnn_state, morph)
            for i, br in enumerate(morph.branches):
                if labels[i] not in (3, 4):
                    continue
                pred = gnn_preds.get(br.branch_id)
                if pred is None:
                    continue
                labels[i], confs[i] = pred
    return labels, confs
...
def morphology_to_branch3_data(
    morph: MorphologyBranches,
    subtree_owner_map: dict[int, dict[str, float | int]],
    current_labels: Sequence[int],
    current_confs: Sequence[float],
) -> Data:
    if not morph.branches:
        x = torch.zeros((0, len(BRANCH3_FEATURE_NAMES)), dtype=torch.float32)
    else:
        x = torch.tensor(
            np.stack(
                [
                    _branch_feature_vector(br, subtree_owner_map, current_labels[i], current_confs[i])
                    for i, br in enumerate(morph.branches)
                ]
            ),
            dtype=torch.float32,
    ...
    data.branch3_mask = mask
    data.file_path = morph.file_path
    data.feature_names = BRANCH3_FEATURE_NAMES
...
def build_graphs(
    ...
    for i, path in enumerate(files):
        try:
            ...
            morph = extract_branches(nodes, "pyramidal", str(path))
            owner_map = _predict_subtree_owner_map(nodes, "pyramidal", owner_model)
            cur_labels, cur_confs = _current_branch_predictions(morph, bundle, owner_map, old_gnn_state)
            graph = morphology_to_branch3_data(morph, owner_map, cur_labels, cur_confs)
...
def _compute_class_weights(graphs: Sequence[Data], device: torch.device) -> torch.Tensor | None:
    mode = (os.environ.get("SWCAL_BRANCH3_CLASS_WEIGHT", "inverse_sqrt") or "off").lower()
    if mode == "off":
        return None
    ...
    if total <= 0:
        return None
    raw = total / (3.0 * np.maximum(totals, 1.0))
...
def _branch3_loss(
    ...
    gamma = float(os.environ.get("SWCAL_BRANCH3_FOCAL_GAMMA", os.environ.get("SWCAL_GNN_FOCAL_GAMMA", "2.0")))
    if gamma > 0.0:
        p_t = log_p_t.exp()
...
def _macro_f1(y_true: np.ndarray, y_pred: np.ndarray) -> tuple[float, float, float, float]:
    if y_true.size == 0:
        return 0.0, 0.0, 0.0, 0.0
    f1s = f1_score(y_true, y_pred, labels=[0, 1, 2], average=None, zero_division=0)
    present = [float(f1s[c]) for c in range(3) if np.any(y_true == c)]
    macro = float(np.mean(present)) if present else 0.0
    return macro, float(f1s[0]), float(f1s[1]), float(f1s[2])
...
def train_one_fold(
    ...
    if class_weight is not None:
        print(f"    Branch3 class weight: {class_weight.tolist()}")
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch in loader:
            ...
            opt.zero_grad()
            logits = model(batch.x, batch.edge_index)
            loss = _branch3_loss(logits, batch.y, batch.sample_weight, class_weight)
            loss.backward()
            opt.step()
        ...
        macro, axon, basal, apical = _macro_f1(yt, yp)
        ...
        if macro > best_macro + 1e-6:
            ...
            best_scores = (macro, axon, basal, apical)
            epochs_since = 0
        else:
            epochs_since += 1
        ...
        if epoch == 0 or (epoch + 1) % 5 == 0:
            print(
                ...
                f"val macro={macro:.4f} ax={axon:.4f} ba={basal:.4f} ap={apical:.4f} "
    ...
    macro, axon, basal, apical = best_scores
...
def fit_final(
    ...
    print(f"\n=== Branch3 final retrain on {len(train_graphs)} files for {n_epochs} epochs ===")
    ...
    print(
        f"\nBranch3 held-out branch metrics: macro={macro:.4f} "
        f"axon={axon:.4f} basal={basal:.4f} apical={apical:.4f} "
        f"cell={metrics['cell_mean_macro_f1']:.4f}"
    )
...
def save_checkpoint(
    ...
    payload = {
        ...
        "feature_names": list(BRANCH3_FEATURE_NAMES),
    ...
    print(f"\nSaved Branch3 checkpoint to {path} ({path.stat().st_size / 1024:.1f} KB)")
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--cache-dir", type=Path, default=None,
                    help="Optional graph cache dir. Defaults to <model-dir>/branch3_cache.")
    ...
    ap.add_argument("--no-old-gnn", action="store_true",
                    help="Build current-label features from Stage2 only, without old basal/apical GNN.")
...
def _cache_path(cache_dir: Path, split: str, quick: bool, no_old_gnn: bool) -> Path:
    suffix = "quick" if quick else "full"
    gnn = "no_old_gnn" if no_old_gnn else "with_old_gnn"
    return cache_dir / f"{split}_{suffix}_{gnn}.pt"
...
def _load_or_build_graphs(
    ...
    graphs = build_graphs(files, bundle, old_gnn_state, progress=True)
    if use_cache:
        cache_path.parent.mkdir(parents=True, exist_ok=True)
        torch.save({"feature_names": list(BRANCH3_FEATURE_NAMES), "graphs": graphs}, cache_path)
...
def main() -> int:
    ...
    ckpt = args.ckpt or (model_dir / "gnn_branch3_rescue.pt")
    cache_dir = (args.cache_dir or (model_dir / "branch3_cache")).resolve()
    cfg = TrainConfig(
        hidden=args.hidden,
    ...
    print(f"Branch3 focal gamma: {os.environ.get('SWCAL_BRANCH3_FOCAL_GAMMA', os.environ.get('SWCAL_GNN_FOCAL_GAMMA', '2.0'))}")
    print(f"Branch3 class weight: {os.environ.get('SWCAL_BRANCH3_CLASS_WEIGHT', 'inverse_sqrt')}")
    ...
    old_gnn_state = None
    old_gnn_path = model_dir / "gnn_apical_basal.pt"
    if not args.no_old_gnn and old_gnn_path.is_file():
        old_gnn_state = load_gnn(old_gnn_path, device=device)
        print(f"Loaded old apical/basal GNN: {old_gnn_path.relative_to(ROOT)}")

    print("\nBuilding Branch3 train graphs...")
    train_graphs = _load_or_build_graphs(
        _cache_path(cache_dir, "train", args.quick, args.no_old_gnn),
        train_files,
        bundle,
        old_gnn_state,
        use_cache=not args.quick,
    )
    print("\nBuilding Branch3 held-out graphs...")
    test_graphs = _load_or_build_graphs(
        _cache_path(cache_dir, "test", args.quick, args.no_old_gnn),
        test_files,
        bundle,
        old_gnn_state,
        use_cache=not args.quick,
    ...
    if args.n_folds <= 1:
        fold_results = []
        # Tiny holdout from train for smoke.

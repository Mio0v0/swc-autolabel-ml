#!/usr/bin/env python3
"""Retrain v12 Stage 2 + GNN with GENTLE class weighting.

Same scaffolding as paper/_retrain_v12_weighted.py but with softer
operating points:
    SWCAL_CLASS_BALANCE_POWER = 1.25   (vs 1.5 in the "aggressive" variant)
    SWCAL_GNN_CLASS_WEIGHT    = inverse_sqrt   (vs "inverse")

Hypothesis: aggressive weighting traded apical F1 for P10. The gentler
variant should keep most of the P10 gain while preserving more of the
apical-class F1.

Outputs to paper/models/v12_weighted_gentle/.
"""
...
OUT_DIR      = ROOT / "paper" / "models" / "v12_weighted_gentle"
...
def main() -> int:
    ...
    print("=== v12 RETRAIN with GENTLE class weighting ===")
    print(f"  SWCAL_CLASS_BALANCE_POWER = {os.environ['SWCAL_CLASS_BALANCE_POWER']}  (Stage 2, gentler than 1.5)")
    print(f"  SWCAL_GNN_CLASS_WEIGHT    = {os.environ['SWCAL_GNN_CLASS_WEIGHT']}     (GNN, gentler than 'inverse')")
    ...
    print(f"\n[1] Re-training Stage 2 with SWCAL_CLASS_BALANCE_POWER=1.25")

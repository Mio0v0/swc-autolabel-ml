#!/usr/bin/env python3
"""Aggregate per-seed lab-subset eval JSONs into a multiseed table.

Mirrors the format of `final_multiseed_v12_branch3_table.txt` so the
lab-subset numbers are directly comparable to the all-corpus row.

Reads:
    paper/results/v12_gt_celltype_seed{42,123,789}_lab_only.json

Writes:
    paper/results/v12_lab_subset_multiseed_summary.json
    paper/results/v12_lab_subset_multiseed_summary.csv
    paper/results/v12_lab_subset_multiseed_table.txt
"""
...
def main() -> int:
    rows = []
    for seed in SEEDS:
        p = RESULTS / f"v12_gt_celltype_seed{seed}_lab_only.json"
        if not p.is_file():
    ...
    if not rows:
        raise SystemExit("No lab-subset JSONs found; run eval first.")

    n_seeds = len(rows)
    summary = {"method": "v12_branch3_gt_celltype_lab_subset",
    ...
    out_json = RESULTS / "v12_lab_subset_multiseed_summary.json"
    out_json.write_text(json.dumps({"per_seed": rows, "summary": summary},
                                    indent=2), encoding="utf-8")
    out_csv = RESULTS / "v12_lab_subset_multiseed_summary.csv"
    with out_csv.open("w", encoding="utf-8", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=list(summary.keys()))
        w.writeheader()
        w.writerow(summary)
    out_txt = RESULTS / "v12_lab_subset_multiseed_table.txt"
    lines = ["Multi-seed v12+Branch3 (GT cell type) -- LAB SUBSET (hpf_ca1)",
             "=" * 64,
             "",
             f"Mean +/- SD over n={n_seeds} seeds ({summary['seeds']}).",
             f"Restricted to cells with filename prefix 'hpf_ca1__'.",
             ""]
    for label, _ in METRICS:

...
def _morphologies_to_per_node_arrays(
    ...
    Weighting policy (chosen by env var SWCAL_SAMPLE_WEIGHT):
      ``node`` (default, backward-compatible): weight = node count.
        Big cells dominate the loss; tiny cells under-represented.
      ``cell``: each cell contributes total weight 1.0, divided across
        its label-count rows by node fraction within the cell.
        Tiny atypical cells get the same total training signal as huge
        typical ones - fixes the worst-decile-P10 cells where Stage 2
        confidently mis-labels small basal-only pyramidals.

    Hard-cell upweighting (Phase 3a exp beta): env var
    SWCAL_HARD_CELLS_JSON points to a JSON with {"hard_cells": [...],
    "upweight_multiplier": float}. Rows from those cells get their weight
    multiplied. Generated by paper._identify_hard_training_cells.
    """
    import os as _os
    import json as _json
    weight_policy = _os.environ.get("SWCAL_SAMPLE_WEIGHT", "node").lower()
    if weight_policy not in ("node", "cell"):
        weight_policy = "node"

    hard_cells: set[str] = set()
    hard_upweight = 1.0
    hard_path = _os.environ.get("SWCAL_HARD_CELLS_JSON")
    if hard_path:
        try:
            payload = _json.loads(Path(hard_path).read_text(encoding="utf-8"))
            hard_cells = set(payload.get("hard_cells", []))
            hard_upweight = float(payload.get("upweight_multiplier", 5.0))
            print(f"  HARD-CELL UPWEIGHT: {len(hard_cells)} cells, weight x{hard_upweight}")
        except Exception as exc:
            print(f"  WARN: failed to load hard_cells JSON {hard_path}: {exc}")
    ...
    for morph in morphologies:
        ...
        # Sum of all valid-neurite node counts in this cell (denominator
        # for cell-equal weighting). Soma is excluded by gt_label_counts.
        cell_total = float(sum(
            count
            for br in morph.branches
        ...
        if cell_total <= 0:
            continue
        # Hard-cell multiplier (Phase 3a exp beta)
        morph_basename = Path(morph.file_path).name
        cell_multiplier = hard_upweight if morph_basename in hard_cells else 1.0
        for br in morph.branches:
            features = _branch_feature_with_owner(br, owner_map)
            for lbl, count in br.gt_label_counts.items():
                if lbl in valid_neurite_labels and count > 0:
                    if weight_policy == "cell":
                        # Each cell contributes total weight 1; split among
                        # its valid-neurite (branch, label) rows by node share.
                        w = float(count) / cell_total
                    else:
                        w = float(count)
                    w *= cell_multiplier
